...
local function handleRiddle(text)
    if not _riddleSolver then return false end
    if not isRiddleText(text) then return false end

    local cacheKey = riddleNormalize(text)
    if _riddleSeenCache[cacheKey] then return true end
    _riddleSeenCache[cacheKey] = true
    task.delay(30, function() _riddleSeenCache[cacheKey] = nil end)
