...
-- ═══════════════════════════════════════════════════════════════
-- OPEN / CLOSE
-- The panel folds down into the die and the die spins up in its place,
-- and the reverse on the way back. Size and Rotation carry it, which
-- leaves the panel's own UIScale — the GUI Scale setting — alone.
-- ═══════════════════════════════════════════════════════════════
do
local miniPop = Instance.new("UIScale")
miniPop.Name = "DiceMiniPop"
miniPop.Scale = 1
miniPop.Parent = MiniFrame
local animating = false
function _G.DicePanelSetOpen(open, instant)
open = open == true
_G.__DiceDuelsMinimized = not open
if instant then
animating = false
Main.Size = FULL_MAIN_SIZE
Main.Rotation = 0
Main.Visible = open
miniPop.Scale = 1
MiniFrame.Rotation = 0
MiniFrame.Visible = not open
return
end
-- A second tap mid-flight would leave the panel at whatever size the
-- running tween had reached, so they are ignored until it settles.
if animating then return end
if open == Main.Visible then return end
animating = true
if open then
tween(MiniFrame, {Rotation = 140}, 0.16)
TweenService:Create(miniPop, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Scale = 0.2}):Play()
...
Main.Rotation = -8
Main.Visible = true
TweenService:Create(Main, TweenInfo.new(0.36, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
Size = FULL_MAIN_SIZE,
Rotation = 0,
}):Play()
-- The dice re-roll as it opens, the same as on a tab change.
...
TweenService:Create(Main, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
...
Main.Rotation = 0
miniPop.Scale = 0.2
MiniFrame.Rotation = -140
MiniFrame.Visible = true
TweenService:Create(miniPop, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
tween(MiniFrame, {Rotation = 0}, 0.3)
...
-- The one colour on the panel, and deliberately so: a live dot reads as
-- live because it is green.
OnlineDot.BackgroundColor3 = Color3.fromRGB(64, 214, 122)
...
-- The lock lives on its Settings row alone now; this just keeps that row
-- in step with the state, whoever changed it.
...
box.Position = UDim2.new(1, -80, 0.5, -16)
...
knob.Name = "Knob"
knob.BackgroundColor3 = Color3.fromRGB(96, 96, 102)
...
-- Both states sit low: the knob carries the read, bright when on and
-- a dim grey when off, over a track that never goes white.
BackgroundColor3 = state and Color3.fromRGB(236, 236, 240) or Color3.fromRGB(96, 96, 102)
})
tween(track, {
BackgroundTransparency = state and 0.02 or 0.05,
BackgroundColor3 = state and Color3.fromRGB(54, 54, 60) or Color3.fromRGB(19, 19, 22)
})
if trackStroke then
tween(trackStroke, {
Color = state and Color3.fromRGB(206, 206, 212) or Color3.fromRGB(64, 64, 70),
Transparency = state and 0.15 or 0.5,
Thickness = state and 1.25 or 1
})
end
if rowStroke then
tween(rowStroke, {
Color = state and Color3.fromRGB(198, 198, 204) or COLORS.strokeSoft,
Transparency = state and 0.28 or 0.38,
...
_G.DicePanelSetOpen(not Main.Visible)
saveDiceConfig()
return
end
...
-- ═══════════════════════════════════════════════════════════════
-- STEAL BAR
-- The Rainy Hub card, in greys: a status dot and AUTO STEAL header on
-- the top line, radius / stats / percentage on the second, and a
-- full-width track with a gradient fill and a knob riding it below.
-- ═══════════════════════════════════════════════════════════════
local BAR_EDGE = Color3.fromRGB(226, 226, 232)
local BAR_GLOW = Color3.fromRGB(248, 248, 252)
local BAR_DIM = Color3.fromRGB(140, 140, 148)
...
pbFrame.BackgroundColor3 = Color3.fromRGB(9, 9, 11)
...
statLbl.TextColor3 = Color3.fromRGB(240, 240, 246)
...
fillRegion.BackgroundColor3 = Color3.fromRGB(16, 16, 19)
...
ColorSequenceKeypoint.new(0, Color3.fromRGB(104, 104, 112)),
ColorSequenceKeypoint.new(0.45, Color3.fromRGB(196, 196, 204)),
ColorSequenceKeypoint.new(1, Color3.fromRGB(252, 252, 255)),
})
...
progressKnob.BackgroundColor3 = Color3.fromRGB(240, 240, 246)
...
barState = state
if state == "STEALING" or state == "READY" then
...
TS:Create(statusDot, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(92, 92, 98)}):Play()
...
_G.DicePanelSetOpen(_G.__DiceDuelsMinimized == true)
saveDiceConfig()
end)
end
